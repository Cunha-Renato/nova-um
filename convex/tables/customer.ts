...
export const updateCustomer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return customer.user_id === user_id ? await ctx.db
      .patch(args.customer_id, {
        name: args.name,
        phone_number: args.phone_number,
        observations: args.observations,
      })
...
async function removeAllTagsCustomerFn(ctx: MutationCtx, customer_id: Id<"customers">) {
  const user_id = await getUserId(ctx);
  if (user_id === null) return null;
...
export async function removeTagAllCustomersFn(ctx: MutationCtx, tag_id: Id<"tags">) {
  const user_id = await getUserId(ctx);
  if (user_id === null) return null;
...
async function removeAllEventDatesCustomerFn(ctx: MutationCtx, customer_id: Id<"customers">) {
  const user_id = await getUserId(ctx);
  if (user_id === null) return null;
...
export async function removeEventDateAllCustomersFn(ctx: MutationCtx, event_date_id: Id<"event_dates">) {
  const user_id = await getUserId(ctx);
  if (user_id === null) return null;
